...
- **An unalignable window replaces only the current segment**
  ...
  - **The bound is one *segment*, not one window** — everything since the
    last measured pause. With no pause in the dictation the floor is empty
    and the old total loss still applies.
...
- **The whole pause mechanism is inert in a room above the silence gate.**
  ...
  session after 20 s without a quiet slice.

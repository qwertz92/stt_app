...
- **An unalignable window replaces only the current segment**
  ...
  later replace can never reach past it. Be precise about what that buys:
  the bound is one **segment** — everything since the last measured pause —
  not one window, and in a dictation with no pause at all the floor is empty
  and the old total loss still applies. Before this, one desk knock could
  cost the whole dictation unconditionally.
...
- The post-pause append gate is an energy measurement, not a real VAD. A
  ...
  is dropped. The two classes genuinely overlap — "Bitte." measures 0.085 s
  and a mechanical key clack 0.080 s — so the gate only filters what it can
  actually distinguish: continuous silence and repetitive keyboard noise.
  An isolated transient passes by design and is bounded by
  `protected_prefix` rather than prevented.

...
def test_settings_combos_keep_native_arrow_rendering():
    app = QtWidgets.QApplication.instance() or QtWidgets.QApplication([])
    dialog = SettingsDialog(
        settings_store=_FakeSettingsStore(AppSettings()),
        secret_store=_FakeSecretStore(),
    ...
    assert type(dialog.language_combo).paintEvent is QtWidgets.QComboBox.paintEvent
    assert "QComboBox::drop-down" not in dialog.styleSheet()

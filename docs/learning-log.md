...
### A stand-in for the Hub, and review round 2 on the download (2026-09-21)

A real download needs the owner's permission, so the lead drove the real
worker and the real huggingface_hub 1.32.0 against an HTTP server on
127.0.0.1 that serves generated bytes at a throttled rate. It confirmed the
new progress path (rate equal to the throttle, monotonic percentage) and found
two defects no unit test and neither review had:

- *huggingface_hub 1.32.0 never resumes a partial file* (upstream PR #4228:
  a process-unique `<etag>.<8 hex>.incomplete`, deleted in a `finally` the
  app's kill never reaches). The orphan was counted by the parent's first
  directory sample and held the percentage at the killed run's value until the
  restart caught up; nothing removed it. It is cleared before the child starts
  and before `snapshot_download`. This arrived with the lead's own dependency
  upgrade two commits earlier, whose measurements (live providers, five
  runtimes) had not included an interrupted download.
- *hub's symlink probe caches `True` before it tests*, so on Windows without
  the symlink privilege a second download thread dies with WinError 1314:
  11 of 12 runs cold against the zero-latency stand-in, 0 of 12 with the probe
  settled first.
- *The first run of the stand-in downloaded a real model.* Its tree endpoint
  answered with the wrong JSON shape, the worker took that for an unreachable
  Hub and fell back to the real ModelScope mirror: 78 MB of `tiny` into the
  temp sandbox, without the permission a download needs. Reported to the
  owner. A probe of the download path sets `STT_APP_DISABLE_MODELSCOPE=1`.

Review round 2 (one read-only reviewer over the round-1 fixes): no P1. The
baseline counted a blob twice where hub had copied it into the snapshot
(Windows without symlinks, `new_blob=False`); the release of a child could
close the spooled error log under a concurrent read and blank the failure
text; a docstring and this file said "five of six" call sites where there are
six of seven; one test asserted nothing; one cap depended on two tables
agreeing with no test saying so. All reproduced, all fixed, each fix with a
negative control.

**Lessons.**
- *A dependency upgrade is measured through every path that uses the
  dependency, including the unhappy ones.* "It downloads" was measured; "it
  downloads again after a kill" was not, and that is where the behaviour had
  changed.
- *A stand-in for a service must not be able to reach the real one.* The
  fallback that makes the app robust is exactly what turns a broken fake into
  real traffic.

...
- **Cloud transcription?** Switch Engine to `AssemblyAI`, `OpenAI`, `Groq`, `Deepgram`, `ElevenLabs`, or `Azure LLM Speech` in Settings and enter your API key. Azure also needs the per-resource **endpoint** (entered next to the keys).
